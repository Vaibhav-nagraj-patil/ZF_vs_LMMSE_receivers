# -*- coding: utf-8 -*-
"""
Created on Fri Jul 28 16:28:39 2023
...
for blk in range(nBlocks):   
    ...
    for K in range(len(SNRdB)):
        TxSym=np.sqrt(Eb[K])*Sym;#Txsymbols after power scaling
        RxSym=np.matmul(H,TxSym)+noise;
        #ZF reciever
        ZFRx=nl.pinv(H);#ZFreciever
        ZFout=np.matmul(ZFRx,RxSym);
        DecBitsI_ZF=(np.real(ZFout)>0);
        DecBitsQ_ZF=(np.imag(ZFout)>0);
        #evaluating total errors for ZF reciever
        ...
        LMMSEout=np.matmul(LMMSERx,RxSym);#output of LMMSE reciever
        DecBitsI_LMMSE=(np.real(LMMSEout)>0);#decoding bits for I channel
        DecBitsQ_LMMSE=(np.imag(LMMSEout)>0);
        #evaluating total number of errors for LLMSE reciever
