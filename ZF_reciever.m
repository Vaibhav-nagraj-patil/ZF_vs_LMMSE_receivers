...
BERt = zeros(1,length(EbdB));%theoretical value

for blk = 1:nBlocks%each block
    H = 1/sqrt(2)*(randn(r,t) + 1j*randn(r,t));%channel matrix coeff are imag and real
    BitsI = randi([0,1],t,blockLength);
    BitsQ = randi([0,1],t,blockLength);
    Sym = (2*BitsI-1)+1j*(2*BitsQ-1);%BPSK is generated 2 is multiplied
...
semilogy(SNRdB,BER_ZF,'g  s','linewidth',1.0,'MarkerFaceColor','g','MarkerSize',5.0);
hold on;
semilogy(SNRdB,BERt,'g -.','linewidth',1.0,'MarkerFaceColor','g','MarkerSize',5.0);
semilogy(SNRdB,BER_LMMSE,'r -- o','linewidth',1.0,'MarkerFaceColor','r','MarkerSize',5.0);
axis tight;
grid on;
